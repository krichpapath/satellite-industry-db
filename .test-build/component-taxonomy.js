...
exports.COMPONENT_SYSTEMS = exports.UNIDENTIFIED_VALUE = exports.SATELLITE_COMPONENT_TAXONOMY = void 0;
exports.cleanSystemLabel = cleanSystemLabel;
exports.cleanComponentLabel = cleanComponentLabel;
exports.normalizeSystem = normalizeSystem;
exports.modulesForSystem = modulesForSystem;
exports.componentsForModule = componentsForModule;
exports.allComponentNames = allComponentNames;
exports.findComponentPath = findComponentPath;
exports.SATELLITE_COMPONENT_TAXONOMY = {
...
exports.UNIDENTIFIED_VALUE = "Unidentified";
const RAW_COMPONENT_SYSTEMS = Object.keys(exports.SATELLITE_COMPONENT_TAXONOMY);
function alphaCompare(a, b) {
    return a.localeCompare(b, undefined, { sensitivity: "base" });
}
function sortedWithUnidentifiedLast(values) {
    return [
        ...values.filter((value) => value !== exports.UNIDENTIFIED_VALUE).sort(alphaCompare),
        exports.UNIDENTIFIED_VALUE
    ];
}
function cleanSystemLabel(system) {
    return String(system ?? "").replace(/^\s*\d+\.\s*/, "").trim();
}
function cleanComponentLabel(component) {
    return String(component ?? "").replace(/^\s*(?:•|â€¢)\s*/, "").trim();
}
function rawSystemFor(system) {
    const value = String(system ?? "").trim();
    return RAW_COMPONENT_SYSTEMS.find((option) => option === value || cleanSystemLabel(option) === value);
}
function normalizeSystem(system) {
    if (String(system ?? "").trim() === exports.UNIDENTIFIED_VALUE)
        return exports.UNIDENTIFIED_VALUE;
    const raw = rawSystemFor(system);
    return raw ? cleanSystemLabel(raw) : String(system ?? "").trim();
}
exports.COMPONENT_SYSTEMS = [
    ...sortedWithUnidentifiedLast(RAW_COMPONENT_SYSTEMS.map(cleanSystemLabel))
];
function modulesForSystem(system) {
    if (normalizeSystem(system) === exports.UNIDENTIFIED_VALUE)
        return [exports.UNIDENTIFIED_VALUE];
    const rawSystem = rawSystemFor(system);
    const modules = rawSystem ? exports.SATELLITE_COMPONENT_TAXONOMY[rawSystem] : undefined;
    return modules ? sortedWithUnidentifiedLast(Object.keys(modules)) : [exports.UNIDENTIFIED_VALUE];
}
function componentsForModule(system, module) {
    if (normalizeSystem(system) === exports.UNIDENTIFIED_VALUE || module === exports.UNIDENTIFIED_VALUE)
        return [exports.UNIDENTIFIED_VALUE];
    const rawSystem = rawSystemFor(system);
    const modules = rawSystem ? exports.SATELLITE_COMPONENT_TAXONOMY[rawSystem] : undefined;
    if (!modules)
        return [exports.UNIDENTIFIED_VALUE];
    const components = [...(modules[module] ?? [])].map(cleanComponentLabel);
    return sortedWithUnidentifiedLast(components);
}
function allComponentNames() {
    return exports.COMPONENT_SYSTEMS.flatMap((system) => modulesForSystem(system).flatMap((module) => componentsForModule(system, module)));
}
function findComponentPath(componentName) {
    const cleanedComponentName = cleanComponentLabel(componentName);
    if (cleanedComponentName === exports.UNIDENTIFIED_VALUE) {
        return { system: exports.UNIDENTIFIED_VALUE, module: exports.UNIDENTIFIED_VALUE };
    }
    for (const system of exports.COMPONENT_SYSTEMS) {
